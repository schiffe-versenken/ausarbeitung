...
Die generelle Funktionsweise von normalem Schiffe-Versenken bleibt erhalten, muss jedoch um folgende Dinge erweitert werden, um auch in höheren Dimensionen gut spielbar zu sein:
...
Diese Spielregeln sind äquivalent zu denen in \cite{M13}.
...
Ein Sampling error $e_v=P(\mathbf{T}^{\Omega_L}_{\strat} \leq v) - P(\mathbf{T}^{L_{all}}_{\strat} \leq v)$ verstärkt sich zwar bei der Berechnung der Verteilungsfunktion der Flotten um ca. den Faktor $2^{e_v |L_{all}|} - 1$, wie aber später in der Auswertung zu sehen ist, macht dies bei den sehr kleinen Werten von $P(\mathbf{X}^{F_{all}}_{\strat} \leq v)$ fast keinen Unterschied.
