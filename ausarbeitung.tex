...
\]
die Menge an allen Zellen, auf welche noch nicht geschossen wurde und noch nicht genau feststeht, wieviele Schiffe bei einem Schuss darauf getroffen werden.
\end{definition}

\begin{definition}
Sei $F\in F^*$ ein Zustand und $c \in C_{undetermined}(F)$ eine Zelle.
Dann ist
\[
...
Dann ist
\[
determined(L, F) \Leftrightarrow distribution\_count(F)=1 \Leftrightarrow C_{undetermined}(F)
\]
...
\begin{align}
&strat:F^* \rightarrow C_{all}\quad mit \nonumber
\end{align}
...
Dann ist
\[
strat_{greedy-hit}(F)=\max_{c \in C_{undetermined}(F)} \mathds{E}(hit(c) \mid F)
\]
...
Dann ist
\[
strat_{greedy-distribution}(F)=\max_{c \in C_{undetermined}(F)} \mathds{E}(distributions\_removed(F,c))
\]
...
\begin{align}
\begin{split}
[0,max\_hit\_count(F, c)] \in eq(F, c_1) \wedge \\
([0,max\_hit\_count(F, c)] \in asc(F, c_2) \lor [0,max\_hit\_count(F, c)] \in desc(F, c_2)) \\
...
Dann ist
\[
strat_{opt}(F)=c \in C_{undetermined}(F) \mid min\_expected\_shots(F, strat_{opt})= \mathds{E}(shots\_left(F, c, strat_{opt}))
\]
...
\min_{c \in C_{left}(F)} \mathds{E}(shots\_left(F, c, strat)) \leq \mathds{E}(shots\_left(F, c_{other}, strat)), c_{other} \in C_{undetermined}(F)\\
\Rightarrow\\
\mathds{E}(shots\_left(F, strat_{opt})) \leq \mathds{E}(shots\_left(F, strat_{alt}))
\end{align}
\qed
