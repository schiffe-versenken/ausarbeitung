...
\begin{algorithm}[H]
 function create\_end\_nodes($F_{start}$):\\
 $nodes=\emptyset$\;
 \For{$L \in distributions(F_{start})$}{
   \For{$F \in determinants(L)$}{
     \If{$F_{start} \subseteq F$}{
...
\begin{algorithm}[H]
 function calculate\_expected\_shots($F_{start}$):\\
 $nodes=create\_end\_nodes(distributions(F))$\;
 $transitions[f,c,h]=\emptyset$\;
 $transitions\_cells[f]=\emptyset$\;
 $results = \emptyset$\;
 \While{$nodes \neq \emptyset$}{
 $previous\_states=\emptyset$\;
 \For{$x \in nodes$}{
   \For{$(c,h) \in (state(F) \setminus F_{start})$}{
...
\begin{algorithm}[H]
 function $strat_{opt}(F_{start}, c$):\\
    \If{$(c,e) \in calculate\_expected\_shots(F_{start}), e \in \mathds{R}$}{
     \Return $-e$\;
   }
   \Return $\infty$ \;
\end{algorithm}
