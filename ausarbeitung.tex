...
\begin{definition}
Sei $\Omega_L \subseteq L_{all}$ eine Menge an Flotten.
\end{definition}
...
Sei $\Omega_L \subseteq L_{all}$ eine Menge aller Flotten.
...
\begin{definition}
Sei $\Omega_F \subseteq F_{all}$ die Menge aller Flotten.
\end{definition}
...
Dann ist
\begin{align}
P(\mathbf{X}^{F(\Omega_L)}_{\strat} \leq v) = \frac{2^l - 1}{2^n - 1}
\nonumber
\end{align}
die Wahrscheinlichkeit, dass eine Flotte, die zufällig aus $F(\Omega_L)$ ausgewählt wurde, nach maximal $v$ Schüssen versenkt wird.
\end{satz}

\begin{proof}
Es gibt $l$ Schiffe, die nach maximal $v$ Schüssen versenkt werden und daher gibt es $|F(L)|=2^l - 1$ Flotten in $F(\Omega_L)$, die nach maximal $v$ Schüssen versenkt werden.

Die Wahrscheinlichkeit, eine Flotte auszuwählen, die in $F(L)$ liegt, beträgt $\frac{2^l - 1}{2^n - 1}$, da es eine Gleichverteilung ist.
\qed
\end{proof}
...
Es werden aus den Ergebnissen von einer Stichprobe mit $n$ Schiffen $\Omega_L$ das Ergebnis für alle möglichen Flotten, die aus den Schiffen der Stichprobe erstellt werden können $F(\Omega_L)$, berechnet. Die Anzahl dieser Flotten beträgt $|F(\Omega_L)|=2^n - 1$. Natürlich ist dieses Ergebnis sehr stark von der Stichprobe an Schiffen abhängig und kann nicht mit einer echten zufälligen Auswahl von $2^n - 1$ Flotten aus $F_{all}$ verglichen werden.
