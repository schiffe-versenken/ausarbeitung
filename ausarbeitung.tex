...
\begin{abstract}
In dieser Ausarbeitung werden zunächst formal das Spielprinzip, Strategien und eine Einbettung für diese in ein Monte-Carlo-Verfahren vorgestellt. Anschließend werden Algorithmen für Schussstrategien mit vollen und dünnen Gittern, Halton- und Sobol-Sequenzen vorgestellt und teilweise ihre Komplexität analysiert. Danach wird deren Implementierung thematisiert. Schließlich werden die Ergebnisse ausgewertet, graphisch dargestellt und die Strategien anhand dessen verglichen um herauszufinden welche Strategie sich am besten zum Versenken eines einzelnen Schiffs bzw. einer Flotte eignet.
\end{abstract}
...
Diese Ausarbeitung beschäftigt sich mit der Umsetzung von Schiffe-Versenken in beliebig vielen Dimensionen.
Die generelle Funktionsweise von normalem Schiffe-Versenken bleibt erhalten, muss jedoch um einige Dinge erweitert werden, um auch in höheren Dimensionen gut spielbar zu sein. Diese Spielregeln sind äquivalent zu denen in \cite{M13}.

Das Spielfeld besteht, analog zum echten Spiel, aus Zellen, auf die jeweils geschossen werden kann. Bloß kann hier das Spielfeld beliebig viele Dimension umfassen und kann somit viel mehr Zellen enthalten, als das echte Spiel, in dem es nur 2 Dimensionen gibt. Die Spielfeldgröße entlang einer Dimension kann ebenfalls beliebig, also auch viel größer, gewählt werden, mit der Einschränkung, dass die Spielfeldgröße der Einfachheit halber in allen Dimensionen gleich groß ist.

Eine Schiffsposition ist ein Hyperquader, der einen Teil des Spielfeldes, also einige Zellen, umfasst. Die Ausdehnung des Hyperquaders muss entlang jeder Dimension mindestens eine Zelle und maximal die Spielfeldgröße sein. Die Menge aller möglichen Schiffspositionen ist also die Menge aller Hyperquader, für die diese Bedingung gilt. Das heißt, dass verschiedene Schiffe sich, im Gegensatz zum orginalen Spiel, entlang jeder Dimension überlappen können.
...
Sobald alle Schiffe der Flotte versenkt wurden, ist die Flotte versenkt und das Spiel beendet.
Das Ziel des Spielers ist es, mit möglichst wenig Schüssen alle Schiffe zu versenken.
Dafür können verschiedene Schuss-Strategien angewendet und miteinander verglichen werden.

\subsection{Inhalt}

Zu Beginn werden die in der Spielbeschreibung genannten Konzepte mit einigen Definitionen formalisiert, damit sie später angewendet werden können.
...
Im dritten Teil werden unter den zuvor etablierten Rahmenbedingungen verschiedene Strategien vorgestellt und implementiert. Zu den Strategien zählen dünne und volle Gitter, klassisches Monte Carlo in Form einer zufälligen Strategie und Quasi Monte Carlo in Form von Halton- und Sobol-Strategien.
...
\]
gültige Schiffspositionen, da für jede Schiffsposition und Dimension gültige Start- und Endkoordinaten gewählt werden müssen, also genau $d$ gültige Start- und Endkoordinaten.
\end{proof}
...
Somit ist $(\Omega_L, P)$ ein endlicher Laplacescher W-Raum und P eine Gleichverteilung.
...
Somit ist $(\Omega_F, P)$ ein endlicher Laplacescher W-Raum und P eine Gleichverteilung.
...
Sei außerdem $L=\{x \in \Omega_L \mid \shot_count(l, \strat) \leq v)\}$ die Menge an Schiffen aus $\Omega_L$, die nach maximal $v$ Schüssen versenkt werden und $l=|L|$.
...
Um nun die Verteilungsfunktion von Flotten zu bestimmen, wird im folgenden nun davon ausgegangen, dass gilt:
\[
...
Ein Sampling error $e_v=P(\mathbf{T}^{\Omega_L}_{\strat} \leq v) - P(\mathbf{T}^{L_{all}}_{\strat} \leq v)$ verstärkt sich zwar bei der Berechnung der Verteilungsfunktion der Flotten um ca. den Faktor $2^{e_v |L_{all}|} - 1$, wie später in der Auswertung zu sehen ist.
...
Noch zu beachten ist, dass Koordinatenlevel bei Gittern die Maschenweite und damit die Genauigkeit angeben. Normalerweise legt man ein Gitterlevel fest und erzeugt sich dann aus Inkrementen ein Gitter eben dieses Levels. In unseren Fall arbeiten wir uns durch die Inkremente bei aufsteigendem Gitterlevel um immer genauere Gitter zu erzeugen, bis wir alle Schiffe der Flotte getroffen haben.
...
Außerdem ist an Abbildung \ref{fig:gitter02} zu sehen, dass in den neu hinzukommenden Punkten mindestens eine neue Koordinaten verwendet wird. Das heißt wir können pro neuer Schicht zu Beginn einmalig die neuen Koordinaten berechnen und anschließend diese einfach aufsteigend miteinander zu Punkten kombinieren. Dabei können Schüsse wie Zahlen in einem Zahlensystem betrachtet werden. Wir denken uns die verwendeten Koordinaten aus denen Schüsse kombiniert werden können als Ziffern eines Zahlensystems. Darin sind Koordinaten eines höheren Koordinatenlevels größer einzuordnen als die eines niedrigeren. Außerdem werden die Koordinaten innerhalb eines Koordinatenlevels von klein nach groß sortiert. Damit ergibt sich eine absolute Ordnung der Punkte. Schüsse dieser Strategie lassen sich durch Inkrementieren der Ziffern des Zahlensystems bestimmen.
...
Mit dieser Funktion wird die erste Koordinate initialisiert. Wenn $K$ leer ist haben wir noch keine Koordinate gespeichert und daher auch noch keinen Schuss abgegeben. Die erste Koordinate, die für das erste volle Gitter benutzt wird ist gerade die Größe des Spielfelds $N$ halbiert. Falls $N$ ungerade ist wird abgerundet. max\_coord enthält die maximal mögliche Koordinate. $L$ beschreibt das maximale Koordinatenlevel von dem unsere Koordinaten in $K$ stammen. Gesamte Laufzeitkomplexität $O(1)$.
...
Da wir in unserer Herangehensweise diskret arbeiten kann es sein, dass das Halbieren nicht immer aufgeht daher muss hier gerundet werden. Es wird aufgerundet, da sonst der Fall eintreten kann, das Punkte nicht erreicht werden können. Dabei wird durch eine Abfrage gewährleistet das keine gleichen Koordinaten aufgenommen werden. Zum Schluss wird die Menge der Koordinaten $K$ aktualisiert und das Koordinatenlevel $L$ erhöht. $K$ kann maximal um $\left\lceil\frac{N}{2}\right\rceil$ viele neue Koordinaten erweitert werden. Das ist gleichzeitig auch die maximal mögliche Größe von $K_{old}$. Also haben wir für die Schleife $O(N)$. 


Der Vergleich im If benötigt $d$ Schritte daher ist die Zeitkomplexität $O\left(N+d\right)$.
...
In diesem Abschnitt wird das Inkrementieren im gedachten Zahlensystem realisiert. Dabei wird von der letzten Ziffer nach links gehend geprüft welche davon den aktuell maximal möglichen Wert hat. Diese Ziffern müssen beim Zählen dann zurück auf Null bzw. die kleinste Koordinate $K[0]$ springen. Sobald die erste Ziffer erreicht wird, die nicht den maximal Wert hat kann diese um eins erhöht werden. Die restlichen Ziffern, falls vorhanden, bleiben gleich. 
...
Um die Abhängigkeit der Laufzeitkomplexität von $C_{all}$ kommen wir nicht herum, da jede Zelle mindestens einmal betrachtet werden muss. Allerdings ist die Speicherplatzkomplexität vermeidbar wie wir später noch sehen werden. 
...
In Abbildung \ref{fig:gitter03} ist zu sehen, dass sich im zweidimensionalen Fall bei steigendem Level der Gitter immer eine Diagonale dazukommt. Für den ersten Algorithmus der dünne Gitter Strategie benötigen wir ein $S'$, welches Tupel aus zwei $d$-dimensionalen Vektoren speichert. Der erste um die Zelle zu beschreiben und der zweite um das Koordinatenlevel aus der die korrespondierende Koordinate stammt zu definieren. Das neue $K'$ speichert nun die Koordinaten nach Koordinatenlevels getrennt in verschiedenen Listen oder Arrays und nicht wie $K$ noch alles in einer Liste/Array. $S'$ und $K'$ sind globale Variablen.

Die Funktionsweise des folgenden Algorithmus leitet sich von der mit dünne Gittern einhergehenden Bedingung $|\underline{m}|_1\leq n+d-1$ an die Inkremente ab. Bei konstanter Dimension erhöht sich pro dazukommende Diagonale $n$ jeweils um eins um das nächste Gitter zu erzeugen. Das heißt von den Indices der Inkremente kann sich maximal eines auf ein neues Koordinatenlevel erhöhen damit die Bedingung weiterhin erhalten bleibt. Also erarbeiten wir ausgehend von den alten die neuen Schüsse, indem einzelne Koordinaten um ein Koordinatenlevel erhöht werden. Dabei ist zu beachten, dass innerhalb eines neuen Koordinatenlevels es mehrere mögliche Koordinaten gibt. 
...
\subsection{Monte-Carlo- / Random-Strategie}

Ähnlich wie Strategien aus der Diskretisierung durch dünne und volle Gitter gewonnen wurden, kann eine Strategie aus der Monte-Carlo Diskretisierung abgeleitet werden. Dabei stellen die gewählten Stützstellen, die bei Gittern noch nach Gesetzmäßigkeiten ausgesucht wurden, die Strategie dar. Da bei Monte-Carlo diese Stützstellen zufällig gewählt werden, tun wir dies auch hier für die Random-Strategie. 
...
Die zufällige Auswahl von Zellen, die nicht bereits beschossen wurden lässt sich leicht realisieren indem man zum Beispiel zuerst alle Zellen einem Array zuweist. Dann die Elemente des Arrays zufällig miteinander vertauscht und schließlich beginnend bei Arrayindex 0 die Zellen beschießt. Auf diese Weise erreicht man eine zufällige Schussfolge.
...
Sie basiert auf Polynomen auf dem $\mathbb{Z}_2$, also Polynomen, deren Koeffizienten aus diesem Feld stammen und daher nur die Werte 0 oder 1 annehmen können.
...
\end{equation}
bestimmt. Die Startwerte der $m_{1,j}$ bis $m_{s_j,j}$ können frei gewählt werden, solange sie $\leq 2^k$ sind. Diese Startwerte wurden für die Implementierung von \cite{JK10} aus der Datei new-joe-kuo-7.21201 für die ersten 15 Dimensionen entnommen. Daraus können dann die directions numbers $v_{k,j}$ berechnet werden mit:
\begin{equation}
v_{k,j} = \frac{m_{k,j}}{2^k}
\end{equation}

Daraus kann schließlich der $i$-te Punkt der Sobol Sequenz berechnet werden mit $i=(\dots b_2 b_1)_2$ also der Binärdarstellung von $i$:
\begin{equation}
x_{i,j}=b_1v_{1,j}\oplus b_2v_{2,j}\oplus\dots
\end{equation}

Der Algorithmus folgt dem in \cite{JK10} gezeigten mit kleineren Änderungen um ihn mit unserer Herangehensweise kompatibel zu machen. Der dort vorgestellte Algorithmus und dessen mitgelieferte Koeffizienten sind allerdings nur für maximal $2^{32}$ Punkte ausgelegt.
...
Die Bestimmung von Halton Punkten basiert auf Primzahlen, die als Basis dienen. Um die $i$-te Zahl der Sequenz zu bestimmen wird zuerst die Basisdarstellung von $i$ bestimmt. Sei für eine Primzahl $p$, die als Basis dient, $i$ gegeben in Basisdarstellung mit Koeffizienten $a_j$ mit $j\in{0,\dots,k}$ so, dass gilt:
\begin{equation}
i = a_0p^0+\dots+a_kp^k
\end{equation}
Um die zum Index $i$ gehörende Zahl der Halton Sequenz $i_{\text{Halton}}$ zu erhalten wird die Basisdarstellung folgendermaßen umgewandelt:
\begin{equation}
i_{\text{Halton}} = a_0p^{-1}+a_1p^{-2} + \dots+a_kp^{-(k+1)}
\end{equation}

\subsubsection{Algorithmus zur Halton Sequenz}
Halton Sequenzen mit beliebiger Basis lassen sich mit folgenden Algorithmus erzeugen. Dabei gibt dieser jeweils nur die $j$-te Zahl der Sequenz aus, wenn $j=index$ ist.
...
\begin{figure}
	\subfigure[Schicht Gitterlevel 0]{\resizebox{36mm}{!}{\includegraphics{figures/FullGridGeneral1}}}
	\subfigure[Schicht Gitterlevel 1]{\resizebox{36mm}{!}{\includegraphics{figures/FullGridGeneral2}}}
	\subfigure[Schicht Gitterlevel 2]{\resizebox{36mm}{!}{\includegraphics{figures/FullGridGeneral3}}}
	\subfigure[Schicht Gitterlevel 3]{\resizebox{36mm}{!}{\includegraphics{figures/FullGridGeneral4}}}
...
Somit erhalten wir eine Laufzeitkomplexität von \\ $O\left(BlockSize\cdot\left(d\left( 2^{\lceil\log_2(N)\rceil}+1\right)+\lceil\log_2(N)\rceil\right)\right)$ für den vollen Gitter Algorithmus ($BlockSize$ beschriebt die Anzahl der Zellen eines Blocks). Die Speicherkomplexität beläuft sich dann auf $O(BlockSize)$. Bei $d$ Dimensionen kann $BlockSize$ variiert werden zwischen $N^1$ und $N^d$. Wobei $N^d$ nur zu empfehlen ist wenn ausreichend Speicher zur Verfügung steht. 
...
\begin{landscape}
	\label{fig:random2}
	\includepdf[landscape=true]{figures/random_dimension_ship.pdf}
\end{landscape}
...
\begin{landscape}
	\label{fig:fullGrid2}
	\includepdf[landscape=true]{figures/fullGrid_dimension_ship.pdf}
\end{landscape}


\begin{landscape}
	\label{fig:fullGrid3}
	\includepdf[landscape=true]{figures/fullGrid_dimension_fleet.pdf}
\end{landscape}
...
Auf Seite \pageref{fig:fullGrid3} ist eine genauere Betrachtung der Erwartungswerte für Flotten zu sehen. Hier wird durch die Vergrößerungen die starke Annäherung an 1 noch einmal hervorgehoben. Die Tendenz zeigt, dass wir pro Dimension $D$ um ca. $+9\cdot10^{-D}$ näher an die 1 kommen.  
...
\begin{landscape}
	...
	\includepdf[landscape=true]{figures/sparseGrid_dimension_ship.pdf}
\end{landscape}

\begin{landscape}
	\label{fig:sparseGrid3}
	\includepdf[landscape=true]{figures/sparseGrid_dimension_fleet.pdf}
\end{landscape}
...
Bei der Schiffsverteilungsfunktion auf Seite \pageref{fig:sparseGrid1} haben wir einen ähnlichen Verlauf wie bisher, allerdings einen etwas steileren Anstieg als noch bei full Grids. Die Flottenverteilungsfunktion verläuft analog zu anderen Strategien mit den typischen Erwartungswerten nahe 1.
...
\begin{landscape}
	\label{fig:halton2}
	\includepdf[landscape=true]{figures/halton_dimension_ship.pdf}
\end{landscape}

\begin{landscape}
	\label{fig:halton3}
	\includepdf[landscape=true]{figures/halton_dimension_fleet.pdf}
\end{landscape}
...
\begin{landscape}
	\label{fig:sobol1}
	\includepdf[landscape=true]{figures/sobol_dimension.pdf}
\end{landscape}

\begin{landscape}
	\label{fig:sobol2}
	\includepdf[landscape=true]{figures/sobol_dimension_ship.pdf}
\end{landscape}

\begin{landscape}
	\label{fig:sobol3}
	\includepdf[landscape=true]{figures/sobol_dimension_fleet.pdf}
\end{landscape}
...
Bevor wir zum Vergleich der Strategien kommen noch ein paar Worte zu auftretenden Fehlern und Einschränkungen, die gemacht werden müssen. 
...
\begin{landscape}
	\label{fig:compare1}
	\includepdf[landscape=true]{figures/comparsion.pdf}
\end{landscape}

\begin{landscape}
	\label{fig:compare2}
	\includepdf[landscape=true]{figures/comparsion_dimension_ship.pdf}
\end{landscape}

\begin{landscape}
	\label{fig:compare3}
	\includepdf[landscape=true]{figures/comparsion_dimension_fleet.pdf}
\end{landscape}

Die Schiffsverteilungsfunktion auf Seite \pageref{fig:compare1} zeigt alle verwendeten Strategien im Vergleich zueinander. Dabei soll durch die Kurven minRandom und maxRandom die Varianz bei der Random-Strategie miteinbezogen werden.
...
In dieser Darstellung lassen sich minRandom und maxRandom kaum unterschieden.

Betrachten wir nun die Flottenverteilungsfunktion auf Seite \pageref{fig:compare1}. Allgemein sind Kurven, die in dieser Abbildung schneller ansteigen besser, da es gewünscht ist möglichst schnell am oberen Rand anzukommen. Dies würde nämlich bedeuten, dass die zufällige Flotte versenkt ist. 
...
Die interessanten Stellen sind nun einerseits der Wechsel von Halton zwischen 0.2 und 0.4. Das bedeutet, dass Halton zuvor schlechter voran gekommen ist als Random und sich in diesem Zeitraum gegenüber der Random-Strategie verbessert hat. Ab 0.8 liegt bei Halton allerdings ein Messfehler vor, da die Kurve zu früh auf 1 ansteigt. Andererseits ist der Vergleich der minRandom und maxRandom Kurven interessant. Im Bereich von 0.8 bis 1.0 liegt maxRandom oberhalb von minRandom was natürlich zu erwarten ist. Allerdings ist der durch die Varianz entstehende Schlauch zwischen den beiden Kurven reicht klein. Die Kurven überlagern sich entweder, oder sie haben nur einen kleinen Abstand zueinander. Das heißt die Varianz bei der Random-Strategie ist nicht allzu groß. Zu beachten ist, dass Sobol in diesem Plot noch außer Konkurrenz ist, da es Sobol für $10^7$ ist.

Dies ist allerdings behoben auf Seite \pageref{fig:compare2}. Hier wurde zum besseren Vergleich für Sobol noch die Kurve ''halD7'' eingefügt, die Halton für $D=7$ darstellt. Aufgrund des engen Verlaufs dieser beiden Kurven in der Abbildung rechts oben ist zu vermuten, dass dies für $D=8$ ebenfalls der Fall ist. Somit können wir uns für Vergleiche Sobol mit $D=8$ einfach nahe Halton denken. Die Abbildung rechts oben ist besonders interessant, da sie eine Vergrößerung um den Nullpunkt zeigt. 
...
Was die Erwartungswerte auf Seite \pageref{fig:compare2} angeht sehen diese wie erwartet aus. Full-Grid gefolgt von sparse-Grid, dann Halton/Sobol und zuletzt Random. 

Auf Seite \pageref{fig:compare3} sind zuletzt noch die Flottenverteilungsfunktionen dargestellt. Hier können noch einmal schön die zuvor hervorgehobenen Stellen, insbesondere oben rechts, betrachtet werden. Etwas neues interessantes ist, dass sich Halton von Sobol löst für $D=7$. Dies lässt vermuten, dass sich Sobol für $D=8$ eher am Verlauf der Random-Strategie orientiert, wovon sich Halton dann ebenfalls lösen würde.

Die gleiche Tendenz zeigen die Erwartungswerte. Wenn wir Sobol für $D=8$ eher bei Random ansiedeln dann zeigt sich Halton mit dem kleinsten Erwartungswert.

\section{Zusammenfassung und Fazit}
In dieser Ausarbeitung wurden zunächst formal das Spielprinzip, Strategien und eine Einbettung für diese in ein Monte-Carlo-Verfahren vorgestellt. Anschließend wurden Algorithmen für volle Gitter, dünne Gitter, Halton- und Sobol-Sequenzen vorgestellt und teilweise auf ihre Komplexität analysiert. Danach wurde deren eventuell vom Algorithmus abweichende Implementierung thematisiert. Abweichungen konnten dabei auftreten wenn die Implementierung  auf Systemen mit beliebig großem Speicher lauffähig sein sollte. Es wurden für verschiedene Dimensionen und Spielfeldgrößen obige Strategien und eine Random-Strategie ausgeführt. Die Ergebnisse wurden ausgewertet, graphisch dargestellt und die Strategien damit verglichen. Dabei hat sich gezeigt, dass um ein einzelnes zufälliges Schiff zu treffen sich die Random-Strategie am besten eignet, die bereits von fast jedem Spieler des echten Spiels benutzt wird. Um eine zufällige Flotte zu versenken eignet sich die Halton-Strategie mit dem geringsten Erwartungswert an Schüssen am besten.
