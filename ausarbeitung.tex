...
\begin{tcolorbox}
	\begin{algorithmic}
		...
		\For{\textbf{each } $k\in K$}
		\State $temp.add(k-min)$
		\State $temp.add(k+min)$
		\EndFor
		\State $K=temp$
		\State $L++$
		\EndIf
		\If{$S=\emptyset$\textbf{ or } $!S[last\_index].contain(Any\_of(K))$}
...
Das Prüfen ob der letzte in $S$ gespeicherte Schuss irgendeine Koordinate aus $K$ enthält benötigt $O\left(\left\lceil\frac{N}{2}\right\rceil\cdot d\right)$ Schritte. Das ist auch die gesamte Komplexität dieses Abschnitts. Hier wird geprüft ob wir uns am Anfang einer neuen Gitter Schicht befinden und der Schuss entsprechend ausgegeben.
